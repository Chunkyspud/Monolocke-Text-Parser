...
routes_sm_zx = [
    "Route 1 / Melemele Sea / Route 1 (Hau’oli Outskirts)",
    "Route 1 (Hau’oli Outskirts)", "Route 3 / Kala’e Bay",
    "Route 2", "Melemele Sea / Hau’oli City (Beachfront)",
    "Hau’oli City (Shopping District) / Hau’oli City (Marina)", "Ten Carat Hill,",
    "Ten Carat Hill (Farthest Hollow)", "Hau’oli Cemetery", "Melemele Meadow",
    "Seaward Cave", "Berry Fields",
    "Verdant Cavern (Trial Site)", "Route 4",
    "Route 5", "Route 6", "Route 7", "Route 8",
    "Route 9", "Hano Beach / Hano Grand Resort", "Paniola Town",
    "Akala Outskirts / Memorial Hill",
    "Diglett’s Tunnel", "Wela Volcano Park",
    "Brooklet Hill,", "Brooklet Hill (Totem’s Den)",
    "Lush Jungle", "Paniola Ranch", "Route 10", "Route 11",
    "Route 13", "Route 14 / Tapu Village",
    "Route 16 / Route 15", "Route 17", "Secluded Shore / Route 12",
    "Malie City / Malie City (Outer Cape)",
    "Haina Desert",
    "Ula’ula Meadow", "Malie Garden", "Mount Hokulani",
    "Blush Mountain", "Ruins of Abundance", "Mount Lanakila",
    "Thrifty Megamart (Abandoned Site)",
    "Poni Wilds", "Poni Breaker Coast / Ancient Poni Path",
    "Poni Grove", "Poni Plains", "Poni Coast", "Poni Gauntlet",
    "Seafolk Village", "Poni Meadow", "Vast Poni Canyon",
    "Vast Poni Canyon", "Resolution Cave",
    "Exeggutor Island", ]
routes_usum_zx = [
"Route 1 / Melemele Sea / Route 1 (Hau’oli Outskirts)",
"Route 1 / Route 1 (Hau’oli Outskirts)", "Route 3 / Kala’e Bay",
"Route 2", "Melemele Sea / Hau’oli City (Beachfront)",
"Hau’oli City (Shopping District) / Hau’oli City (Marina)", "Ten Carat Hill,",
"Ten Carat Hill (Farthest Hollow)", "Hau’oli Cemetery", "Melemele Meadow",
"Seaward Cave", "Berry Fields", "Sandy Cave",
"Verdant Cavern (Trial Site)", "Route 1 (Trainers’ School)", "Route 4",
"Route 5", "Route 6", "Route 7", "Route 8",
"Route 9", "Hano Beach / Hano Grand Resort", "Paniola Town",
"Dividing Peak Tunnel", "Akala Outskirts / Memorial Hill",
"Diglett’s Tunnel", "Wela Volcano Park",
"Brooklet Hill,", "Brooklet Hill (Totem’s Den)",
"Lush Jungle","Paniola Ranch", "Route 10", "Route 11",
"Ula’ula Beach", "Route 13", "Route 14 / Tapu Village",
"Route 16 / Route 15", "Route 17", "Route 12",
"Malie City / Malie City (Outer Cape)","Haina Desert",
"Ula’ula Meadow", "Malie Garden", "Mount Hokulani",
"Blush Mountain", "Ruins of Abundance", "Mount Lanakila", "Thrifty Megamart (Abandoned Site)",
"Poni Wilds", "Poni Breaker Coast / Ancient Poni Path",
"Poni Grove", "Poni Plains", "Poni Coast", "Poni Gauntlet",
"Seafolk Village", "Poni Meadow", "Vast Poni Canyon", "Resolution Cave","Exeggutor Island",
]
routelist = []
randomizer = ['Universal', 'ZX']
#######################################
# --------------------------------------
#######################################
# Functions
...
def create_main_window(settings):
    ...
    layout = [[Sg.Menu(menu_def, )],
              ...
              [Sg.Text('Pokémon Type'), Sg.Combo(pkmntype_list, default_value=settings['default type'], key='-PKTYPE-'),
               ...
               Sg.Text('Randomizer Used'), Sg.Combo(randomizer, default_value=randomizer[0], key='-RAND-',
                                                    tooltip='Leave as default if unsure.\n'
                                                            'ZX version is made by Ajarmar on GitHub.')
               ],
...
def main():
    ...
    while True:  # Event Loop
        ...
        if event == 'Run':
            ...
            filename = 'none'
            try:
                filename = "%s/%s/%s-%s (%s).txt" % (values['-OUTPUT FOLDER-'], game, game, pkmntype, values['-RAND-'])
            ...
            # Find encounters and print matches
            def encounters(string):
                if values['-RAND-'] == 'ZX':
                    zx = True
                else:
                    zx = False
                ...
                if not zx:
                    enclist = string.split(') - ')
                    enclist = enclist[1].split(', ')
                    route = string
                    n = 0
                    # Populate pokemon in the route
                    while n < len(enclist):
                        for x in enclist:
                            x = re.sub(" .*", '', x)
                            n += 1
                            encdict[n] = x
                        # Check position for rarity
                    ...
                    enclist = string[1:]
                    n = 0
                    if game in ["Sun", "Moon", "Ultra Sun", "Ultra Moon", ]:
                        sos_filter = [mon for mon in enclist if 'sos' in mon]
                        enclist = [re.sub(' lvs \d.*', '', mon) for mon in enclist if 'sos' not in mon]
                        while n < len(enclist):
                            for x in enclist:
                                n += 1
                                encdict[n] = x
                        ...
                        for x, y in encdict.items():
                            for z in pkmn_list:
                                if z.lower() == y:
                                    raritylist.append(x)
                                    break
                # Check if route has valid type if not exit function
                if len(raritylist) == 0:
                    return
                if game in ['Red', 'Blue', 'Yellow']:
                    floors = [
                        ...
                        ['SEAFOAM ISLANDS (5)'],
                        # F1
                        ['MT.MOON (1)', 'VICTORY ROAD (1)', '[PK] MANSION (1)', 'CERULEAN CAVE (1)',
                         'SEAFOAM ISLANDS (1)'],
                        # F2
                        ...
                        for y in x:
                            if y.lower() in route:
                                if floors.index(x) == 0:
                                    ...
                                    level = "B4F - "
                                elif floors.index(x) == 4 and 'Route 23 ' not in route:
                                    level = "1F - "
                                elif floors.index(x) == 5 and 'Route 23 ' not in route:
                                    level = "2F - "
                    ...
                    floors = [
                        ...
                        ['Set #34 - BURNED TOWER Grass/Cave (Morning)', 'Set #35 - BURNED TOWER Grass/Cave (Day)',
                         ...
                         'Set #36 - WHIRL ISLANDS', 'Set #34 - WHIRL ISLANDSSet #34 - WHIRL ISLANDS',
                         "Set #27 - ICE PATH",
                        ...
                        ["Set #1 - SPROUT TOWER Grass/Cave ", "Set #2 - SPROUT TOWER Grass/Cave",
                         ...
                         'Set #16 - UNION CAVE', 'Set #19 - SLOWPOKE WELL', 'Set #22 - MT.MORTAR',
                         'Set #23 - MT.MORTAR',
                         'Set #24 - MT.MORTAR', 'Set #26 - ICE PATH', 'Set #31 - WHIRL ISLANDS',
                         'Set #32 - WHIRL ISLANDS',
                    ...
                    floors = [
                        ...
                        ['Set #26 - GRANITE CAVE', 'Set #37 - GRANITE CAVE', 'Set #13 - METEOR FALLS',
                         ...
                         'Set #34 - MT. PYRE', 'Set #39 - MT. PYRE', 'Set #57 - CAVE OF ORIGIN',
                         'Set #61 - VICTORY ROAD',
                         'Set #77 - NEW MAUVILLE',
                        ...
                        # Inner Room
                        ['Set #70 - SHOAL CAVE', 'Set #71 - SHOAL CAVE', 'Set #72 - SHOAL CAVE',
                         'Set #156 - SHOAL CAVE',
                    ...
                    floors = [
                        ...
                        ["Set #145 - Burned Tower", "Set #146 - Burned Tower", "Set #147 - Burned Tower",
                         ...
                         "Set #245 - Mt. Mortar", "Set #246 - Mt. Mortar", "Set #247 - Mt. Mortar",
                         "Set #248 - Mt. Mortar",
                         "Set #249 - Mt. Mortar", "Set #331 - Mt. Mortar", "Set #332 - Mt. Mortar",
                         "Set #333 - Mt. Mortar",
                         "Set #334 - Mt. Mortar", "Set #335 - Mt. Mortar", "Set #336 - Mt. Mortar",
                         "Set #337 - Mt. Mortar",
                         ...
                         "Set #98 - Union Cave", "Set #99 - Union Cave", "Set #100 - Union Cave",
                         "Set #101 - Union Cave",
                         "Set #102 - Union Cave", "Set #103 - Union Cave", "Set #104 - Union Cave",
                         "Set #105 - Union Cave",
                        ...
                        ["Set #275 - Ice Path", "Set #276 - Ice Path", "Set #277 - Ice Path", "Set #373 - Ice Path",
                         ...
                         "Set #85 - Union Cave", "Set #86 - Union Cave", "Set #106 - Union Cave",
                         "Set #107 - Union Cave",
                         "Set #108 - Union Cave", "Set #109 - Union Cave", "Set #110 - Union Cave",
                         "Set #111 - Union Cave",
                        ...
                        ["Set #33 - Sprout Tower", "Set #34 - Sprout Tower", "Set #35 - Sprout Tower",
                         ...
                         "Set #148 - Bell Tower",
                         "Set #149 - Bell Tower", "Set #150 - Bell Tower", "Set #200 - Bell Tower",
                         "Set #201 - Bell Tower",
                         ...
                         "Set #226 - Mt. Mortar", "Set #227 - Mt. Mortar", "Set #228 - Mt. Mortar",
                         "Set #229 - Mt. Mortar",
                         "Set #230 - Mt. Mortar", "Set #231 - Mt. Mortar", "Set #232 - Mt. Mortar",
                         "Set #308 - Mt. Mortar",
                         "Set #309 - Mt. Mortar", "Set #310 - Mt. Mortar", "Set #311 - Mt. Mortar",
                         "Set #312 - Mt. Mortar",
                         "Set #313 - Mt. Mortar", "Set #314 - Mt. Mortar", "Set #315 - Mt. Mortar",
                         "Set #316 - Mt. Mortar",
                         ...
                         "Set #491 - Mt. Silver Cave", "Set #492 - Mt. Moon", "Set #493 - Mt. Moon",
                         "Set #494 - Mt. Moon",
                        ...
                        ["Set #402 - Mt. Silver Cave", "Set #403 - Mt. Silver Cave", "Set #404 - Mt. Silver Cave",
                         'Set #151 - Bell Tower', 'Set #152 - Bell Tower', 'Set #153 - Bell Tower',
                         'Set #205 - Bell Tower',
                         'Set #206 - Bell Tower', 'Set #207 - Bell Tower', 'Set #208 - Bell Tower',
                         'Set #209 - Bell Tower',
                         ],
                        # 4F
                        ["Set #366 - Mt. Silver Cave", "Set #367 - Mt. Silver Cave", "Set #368 - Mt. Silver Cave",
                         ...
                         'Set #156 - Bell Tower',
                         'Set #210 - Bell Tower', 'Set #211 - Bell Tower', 'Set #212 - Bell Tower',
                         'Set #213 - Bell Tower',
                         'Set #214 - Bell Tower',
                         ],
                        # 5F
                        ['Set #157 - Bell Tower', 'Set #158 - Bell Tower', 'Set #159 - Bell Tower',
                         'Set #215 - Bell Tower',
                         'Set #216 - Bell Tower', 'Set #217 - Bell Tower', 'Set #218 - Bell Tower',
                         'Set #219 - Bell Tower',
                         ],
                        # 6F
                        ['Set #160 - Bell Tower', 'Set #161 - Bell Tower', 'Set #162 - Bell Tower',
                         'Set #220 - Bell Tower',
                         'Set #221 - Bell Tower', 'Set #222 - Bell Tower', 'Set #223 - Bell Tower',
                         'Set #224 - Bell Tower',
                         ],
                        # 7F
                        ['Set #163 - Bell Tower', 'Set #164 - Bell Tower', 'Set #165 - Bell Tower',
                         'Set #225 - Bell Tower',
                         'Set #226 - Bell Tower', 'Set #227 - Bell Tower', 'Set #228 - Bell Tower',
                         'Set #229 - Bell Tower',
                         ],
                        # 8F
                        ['Set #166 - Bell Tower', 'Set #167 - Bell Tower', 'Set #168 - Bell Tower',
                         'Set #230 - Bell Tower',
                         'Set #231 - Bell Tower', 'Set #232 - Bell Tower', 'Set #233 - Bell Tower',
                         'Set #234 - Bell Tower',
                         ],
                        # 9F
                        ['Set #169 - Bell Tower', 'Set #170 - Bell Tower', 'Set #171 - Bell Tower',
                         'Set #235 - Bell Tower',
                         'Set #236 - Bell Tower', 'Set #237 - Bell Tower', 'Set #238 - Bell Tower',
                         'Set #239 - Bell Tower',
                         ],
                        # 10F
                        ['Set #382 - Bell Tower', 'Set #383 - Bell Tower', 'Set #384 - Bell Tower',
                         'Set #512 - Bell Tower',
                        ...
                        ['Set #309 - Dark Cave', 'Set #310 - Dark Cave', 'Set #311 - Dark Cave', 'Set #312 - Dark Cave',
                         ...
                         'Set #422 - Dark Cave', 'Set #423 - Dark Cave', 'Set #424 - Dark Cave',
                         'Set #425 - Dark Cave', ],
                    ]
                    for x in floors:
                        for y in x:
                    ...
                    floors = [
                        ...
                        # Outside
                        ['Set #15', 'Set #16', 'Set #29', 'Set #80', 'Set #81', 'Set #82', 'Set #83', 'Set #84',
                         'Set #85',
                         'Set #86',
                        ...
                        for y in x:
                            if y.lower() in route:
                                if floors.index(x) == 0:
                                    ...
                                    if y == 'Set #60':
                                        level = "1F (West) - "
                                    elif y in ['Set #104', 'Set #105', 'Set #106', 'Set #107', 'Set #108',
                                               'Set #109', ]:
                                        level = "1F (Right Side)"
                    ...
                    floors = [
                        ...
                        # Giant Chasm
                        ['Set #76', 'Set #77', 'Set #78', 'Set #79', 'Set #80', 'Set #81', 'Set #82', 'Set #83',
                         'Set #84',
                         'Set #85',
                         'Set #86', 'Set #87', 'Set #88', 'Set #89', 'Set #90', 'Set #91', 'Set #92', 'Set #93',
                         'Set #94',
                         'Set #95',
                         'Set #96',
                         ],
                ...
                # Show chance to catch
                if "grass/cave" in route or (
                        game in ['Red', 'Blue', 'Yellow'] and ('grass/cave' in route or 'surfing ' in route)) \
                        or 'shaking spots' in route or 'double grass' in route:
                    for x in raritynumbers:
                        if x in ["1/12", "2/12", "1/10", "2/10"]:
                            walkingpercent.append("20%")
                        elif x in ["3/10"]:
                            walkingpercent.append("15%")
                        elif x in ["3/12", "4/12", "5/12", "6/12", "4/10", "5/10", "6/10"]:
                            walkingpercent.append("10%")
                        elif x in ["7/12", "8/12", "7/10", "8/10"]:
                            walkingpercent.append("5%")
                        elif x in ["9/12", "10/12", "9/10"]:
                            walkingpercent.append("4%")
                        elif x in ["11/12", "12/12", "10/10"]:
                            walkingpercent.append("1%")
                        # Gold Silver Crystal Rates
                        elif x in ["1/7", "2/7"]:
                            walkingpercent.append("30%")
                        elif x == "3/7":
                            walkingpercent.append("20%")
                        elif x == "4/7":
                            walkingpercent.append("10%")
                ...
                elif "surfing" in route or 'surf (' in route:
                    for x in raritynumbers:
                        if x == "1/5":
                            if game in ["X", "Y", "Omega Ruby", "Alpha Sapphire", "Sun", "Moon", "Ultra Sun",
                                        "Ultra Moon", ]:
                                surfingpercent.append("50%")
                            else:
                                surfingpercent.append("60%")
                        elif x == "2/5":
                            surfingpercent.append('30%')
                        elif x == "3/5":
                            if game in ["X", "Y", "Omega Ruby", "Alpha Sapphire", "Sun", "Moon", "Ultra Sun",
                                        "Ultra Moon", ]:
                                surfingpercent.append("15%")
                            else:
                                surfingpercent.append("5%")
                        elif x == "4/5":
                            surfingpercent.append("4%")
                        elif x == "5/5":
                            surfingpercent.append("1%")
                        # Gold Silver Crystal Rates
                        elif x == "1/3":
                            surfingpercent.append("60%")
                        elif x == "2/3":
                            ...
                            surfingpercent.append("10%")

                elif "old rod" in route or "good rod" in route or "super rod" in route or "fishing" in route:
                    for x in raritynumbers:
                        if x == "1/5":
                            fishingpercent.append("40%")
                        elif x == "2/5":
                            fishingpercent.append('30%')
                        elif x == "3/5":
                            fishingpercent.append("15%")
                        elif x == "4/5":
                            fishingpercent.append("10%")
                        elif x == "5/5":
                            fishingpercent.append("5%")
                        # Red, Blue, Yellow Rates
                        elif x in ["1/1"]:
                            fishingpercent.append("100%")
                        elif x in ["2/2", "1/2"]:
                            fishingpercent.append('50%')
                        elif x in ["1/4", "2/4", "3/4", "4/4"]:
                            fishingpercent.append("25%")
                        # FireRed, LeafGreen Rates
                        elif x == "1/10":
                            fishingpercent.append("70%")
                        elif x == "2/10":
                            fishingpercent.append('30%')
                        elif x == "3/10":
                            fishingpercent.append("60%")
                        elif x == "4/10":
                            fishingpercent.append("20%")
                        elif x == "5/10":
                            fishingpercent.append("20%")
                        elif x == "6/10":
                            fishingpercent.append("40%")
                            ...
                            fishingpercent.append("4%")
                        elif x == "10/10":
                            fishingpercent.append("1%")
                        # Gen 6 Rates
                        elif x == "1/3":
                            fishingpercent.append('60%')
                        elif x == "2/3":
                            fishingpercent.append("35%")
                        elif x == "3/3":
                            fishingpercent.append("5%")
                elif "rock smash" in route:
                    for x in raritynumbers:
                        if x == "1/5":
                            rocksmashpercent.append("60%")
                        elif x == "2/5":
                            rocksmashpercent.append("30%")
                        elif x == "3/5":
                            rocksmashpercent.append("5%")
                        elif x == "4/5":
                            rocksmashpercent.append("4%")
                        elif x == "5/5":
                            rocksmashpercent.append("1%")
                if "(morning)" in route:
                    time_of_day = "(Morning) - "
                elif "(day)" in route:
                    time_of_day = "(Day) - "
                    ...
                    time_of_day = "(Night) - "
                if 'table' in route:
                    table = re.findall('table \d+', route)[0]
                if game in ["Sun", "Moon", "Ultra Sun", "Ultra Moon", ]:
                    for items in range(len(raritynumbers)):
                        if raritynumbers[items] in ['1/10', '2/10']:
                            raritynumbers[items] = '20%'
                        elif raritynumbers[items] in ['3/10', '4/10', '5/10', '6/10', '7/10']:
                            raritynumbers[items] = '10%'
                        elif raritynumbers[items] == '8/10':
                            raritynumbers[items] = '5%'
                        elif raritynumbers[items] == '9/10':
                            raritynumbers[items] = '4%'
                        elif raritynumbers[items] == '10/10':
                            raritynumbers[items] = '1%'
                        elif raritynumbers[items] == '1/4':
                            raritynumbers[items] = '78%'

                    f.write(table + ' ' + time_of_day + ': Frequency: ' + frequencynumber + ' | Rarity: ' + str(
                        raritynumbers) + '\n')
                    return
                if "grass/cave" in route:
                    f.write(
                    ...
                    f.write(
                        level + time_of_day + "Surfing" + ": " + "Frequency: " + frequencynumber + " | Rarity: " + str(
                            surfingpercent) + "\n")

                elif "old rod" in route:
                    f.write(
                        level + time_of_day + "Old rod" + ": " + "Frequency: " + frequencynumber + " | Rarity: " + str(
                            fishingpercent) + "\n")

                elif "good rod" in route:
                    f.write(
                        level + time_of_day + "Good rod" + ": " + "Frequency: " + frequencynumber + " | Rarity: " + str(
                            fishingpercent) + "\n")

                elif "super rod" in route:
                    f.write(
                        level + time_of_day + "Super rod" + ": " + "Frequency: " + frequencynumber + " | Rarity: " + str(
                            fishingpercent) + "\n")

                elif "fishing" in route:
                    f.write(
                        level + time_of_day + "Fishing" + ": " + "Frequency: " + frequencynumber + " | Rarity: " + str(
                            fishingpercent) + "\n")

                elif "rock smash" in route:
                    f.write(
                        level + time_of_day + "Rock Smash" + ": " + "Frequency: " + frequencynumber + " | Rarity: " + str(
                            rocksmashpercent) + "\n")

                elif "common horde" in route:
                    f.write(
                        level + time_of_day + "Common Horde" + ": " + "Frequency: " + frequencynumber + " | Rarity: 60%\n")

                elif "uncommon horde" in route:
                    f.write(
                        level + time_of_day + "Uncommon Horde" + ": " + "Frequency: " + frequencynumber + " | Rarity: 35%\n")

                elif "rare horde" in route:
                    f.write(
                        level + time_of_day + "Rare Horde" + ": " + "Frequency: " + frequencynumber + " | Rarity: 5%\n")

                elif "swarm" in route:
                    f.write(level + time_of_day + "Swarm/Radar/GBA" + "\n")
                ...
                if hastype == "YES":
                    f.write(
                        table_name + level_range + ": Frequency: " + frequencynumber + " | Rarity: " + str(
                            raritynumbers) + '\n')
            # For Generation 6
            def encounters_3ds(string):
                encounters_ref = string.splitlines()
                encounters_ref = encounters_ref[2:]
                raritylist = []
                for line in encounters_ref:
                    if '---' in line:
                        continue
                    if 'swarm: ?' in line:
                        continue
                    enc_type = line.split(':')[0]
                    pokemon = re.split(':', line, 1)[1]
                    pokemon = pokemon.split('),')
                    total = 0
                    raritylist = []
                    for mon in pokemon:
                        if 'horde' in enc_type:
                            catch_rate = re.findall('\((.*)\)', enc_type)[0]
                            name = re.findall(r'(.*) \(', mon)[0]
                            name = name.replace(' ', '')
                        else:
                            catch_rate = re.findall('\d.*%', mon)[0]
                            name = re.findall(r'- (.*) \(', mon)[0]
                        forme = mon.split(': ')[1]
                        forme = forme.replace(')', '')
                        for poke in pkmn_list:
                            if '(Forme' in poke:
                                name = name + ' (forme ' + forme + ')'
                            if name.lower() == poke.lower():
                                raritylist.append(catch_rate)
                                total += 1
                    frequency = str(total) + '/' + str(len(pokemon))
                    if total == 0:
                        break
                    if 'horde' in enc_type:
                        f.write(enc_type.capitalize() + ' Frequency: ' + frequency + '\n')
                    else:
                        f.write(enc_type.capitalize() + ' Frequency: ' + frequency + ' Rarity: ' + str(raritylist) + '\n')

            # Find static encounters and check them
            def static(string):
                if 'Unchanged' in string:
                    return
                else:
            ...
            # Search for routes and isolate
            def pokemonsearch(routelist_ref, lines_ref):
                window['progbar'].update_bar(0, max=len(routelist_ref) - 1)
                counter = 0
                if game in ["Sun", "Moon", "Ultra Sun", "Ultra Moon", ] and values['-RAND-'] != 'ZX':
                    while counter < len(routelist_ref):
                        window['progbar'].update_bar(counter)
                        pattern = re.findall(re.escape(routelist_ref[counter]) + r'.*?Map', lines_ref, re.S)
                        ...
                        for tables in night_tables:
                            ...
                            counter += 1
                elif game in ["X", "Y", "Omega Ruby", "Alpha Sapphire", "Sun", "Moon", "Ultra Sun", "Ultra Moon", ]:
                    while counter < len(routelist_ref):
                        window['progbar'].update_bar(counter)
                        route = routelist_ref[counter]
                        if values['-RAND-'] == 'ZX':
                            if game not in ["Sun", "Moon", "Ultra Sun", "Ultra Moon", ]:
                                route = route[10:]
                            search = re.findall(re.escape(route) + r'[\s\S]*?(?=\n.*?\nset|$)', lines_ref, re.I)
                        else:
                            search = re.findall(re.escape(route) + r'[\s\S]*?(?=\n.*?\nmap|$)', lines_ref, re.I)
                        map_num = route[:7]
                        if values['-RAND-'] != 'ZX':
                            ...
                            if game in ['X', 'Y']:
                                ...
                                route7_list = ["Map 328", ]
                                if map_num in route1_list:
                                    route = route + " (1)"
                                elif map_num in route2_list:
                                    route = route + " (2)"
                                elif map_num in route3_list:
                                    route = route + " (3)"
                                elif map_num in route4_list:
                                    route = route + " (4)"
                                elif map_num in route5_list:
                                    route = route + " (5)"
                                elif map_num in route6_list:
                                    route = route + " (6)"
                                elif map_num in route7_list:
                                    route = route + " (7)"
                            elif game in ['Omega Ruby', 'Alpha Sapphire']:
                                route1_list = ['Map 026', 'Map 078', 'Map 035', 'Map 038', 'Map 071', 'Map 046',
                                               'Map 048',
                                               'Map 164',
                                               'Map 219',
                                               'Map 086', 'Map 053', 'Map 057', 'Map 055', 'Map 112', 'Map 058',
                                               'Map 059',
                                               'Map 145',
                                               'Map 123', ]
                                route2_list = ['Map 027', 'Map 079', 'Map 039', 'Map 072', 'Map 047', 'Map 049',
                                               'Map 165',
                                               'Map 220',
                                               'Map 087',
                                               'Map 065', 'Map 068', 'Map 099', 'Map 066', 'Map 113', 'Map 069',
                                               'Map 070',
                                               'Map 146',
                                               'Map 124', ]
                                route3_list = ['Map 080', 'Map 037', 'Map 073', 'Map 166', 'Map 221', 'Map 088',
                                               'Map 100',
                                               'Map 114',
                                               'Map 125', ]
                                ...
                                if map_num in route1_list:
                                    ...
                                    route = route + " (7)"
                                if map_num in route8_list:
                                    route = route + " (8)"
                                elif map_num in route9_list:
                                    route = route + " (9)"
                                elif map_num in route10_list:
                                    route = route + " (10)"
                                elif map_num in route11_list:
                                    route = route + " (11)"
                                elif map_num in route12_list:
                                    route = route + " (12)"
                        route = route.replace(',', '')
                        f.write(f'\n' + '=' * 86 + '\n' + route.center(86, " ") + '\n' + '=' * 86 + '\n')
                        if values['-RAND-'] == 'ZX':
                            ...
                            counter += 1
                    if values['-RAND-'] == 'ZX':
                        route = 'Static Pokemon'
                        f.write(f'\n' + '=' * 86 + '\n' + route.center(86, " ") + '\n' + '=' * 86 + '\n')
                        if game in ["Sun", "Moon", "Ultra Sun", "Ultra Moon", ]:
                            staticpoke = re.search(r'static[\s\S]*?(?=\n.*?totem|$)', lines_ref, re.I)
                        else:
                            staticpoke = re.search(r'static[\s\S]*?(?=\n.*?wild|$)', lines_ref, re.I)
                        staticpoke = staticpoke[0]
                        static(staticpoke)
                else:
                    staticpoke = re.search(r'static[\s\S]*?(?=\n.*?wild|$)', lines_ref, re.I)
                    staticpoke = staticpoke[0]
                    encounter = re.search(r'wild[\s\S]*?(?=\n.*?\n--tm|$)', lines_ref, re.I)
                    encounter = encounter[0]
                    while counter < len(routelist_ref):
                        route = routelist_ref[counter]
                        search = re.findall(r'set #[\d]*?.*' + re.escape(route) + r'[\s\S]*?(?=\n.*?\nset|$)',
                                            encounter, re.I)
                        f.write(f'\n' + '=' * 86 + '\n' + route.center(86, " ") + '\n' + '=' * 86 + '\n')
            ...
            try:
                with open(values['-LOG-'], encoding="utf8") as file:
                    lines = file.read()
                    pokemonsearch(routelist, lines)
                    file.close()
